...
export function MapFiltersBar() {
  ...
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 flex flex-col gap-2 items-center">
      {/* Selector de ciudad */}
      <button className="flex items-center gap-1.5 px-4 py-2 bg-white rounded-full shadow-md text-sm font-medium hover:bg-gray-50">
        <span>📍</span>
        <span>Mi ciudad</span>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 max-w-2xl px-2">
        {CATEGORIES.map((cat) => (
          <button
            key={cat.id}
            onClick={() =>
